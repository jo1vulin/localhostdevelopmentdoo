...
function sheet() {
  ...
  if (!s) {
    ...
    s.appendRow(['at', 'name', 'score', 'killed', 'won', 'theme', 'stage']);
  }
  return s;
}
...
function readAll() {
  ...
  return rows.filter(r => r[1]).map(r => ({ at: r[0] instanceof Date ? r[0].toISOString() : String(r[0]), name: String(r[1]), score: Number(r[2]), killed: Number(r[3]), won: r[4] === true || r[4] === 'TRUE', theme: String(r[5] || 'paper'), stage: Number(r[6] || 1) }));
}
...
function doPost(e) {
  ...
  try {
    const at = new Date();
    sheet().appendRow([at, name, score, killed, body.won === true && stage === MAX_STAGE, ['paper', 'hearth', 'cyber'].includes(body.theme) ? body.theme : 'paper', stage]);
