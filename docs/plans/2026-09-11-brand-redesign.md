...
- **Battle City.** Type `battlecity` anywhere on the page (or run it in the terminal, or open `/#battlecity`). `js/battle.js` loads on demand and turns the current viewport into a Battle City map: every visible line of text becomes a row of bricks where it sits (text nodes measured with `Range.getClientRects`), buttons, portraits and logos become steel, the seal becomes a steel ring with bricks inside, and the base is a small stamp in a brick shelter at the bottom. Eleven stages, each built from a different slice of the page (the game scrolls the page underneath and rescans it). Twenty enemies per stage in four types; each stage shifts the mix toward fast, power and armour tanks, speeds them up, and allows more on screen. The player gains speed every stage and a permanent star every three. Flashing enemies (the 4th, 11th and 18th) drop Battle City bonuses: helmet (10 s shield), star (faster shots, then two shots, then shots that break steel), shovel (steel around the base for 20 s), timer (freeze 8 s), grenade (clear the screen), tank (extra life), 500 points each. Spawn stars, bricks break two tiles wide, three lives with a spawn shield, WebAudio sounds. The scoreboard records score, tanks and the stage reached. Arrows or WASD, Space, P, M, R, Esc; on touch devices the map takes the top of the screen and a D-pad, pause, quit and a fire button (auto-fire while held) sit below it. Drawn in the active theme's colours. The `t` and backtick shortcuts are suspended while a round is on.
- **Scoreboard.** At the end of a round with a score, the player enters a name (remembered for next time) and the result goes to the scoreboard: top 10 with rank, name, score and tanks, the player's own entry highlighted; `scores` in the terminal prints it. Persistence is behind one meta tag: while `<meta name="score-api">` is empty the board lives in the visitor's browser; with an endpoint it is shared worldwide and survives deployments. The endpoint is a three-call contract (GET top 10, POST start to open a session, POST an entry) implemented as a Cloudflare Worker with KV in `scoreboard/`. A submission needs an unused session token issued when the round started, enough real time elapsed since then for the result to be possible, an HMAC signature over the score, and the usual plausibility limits; a hand-written POST fails on the first check. Results are also bounded by the elapsed time (points and tanks per stage the time allows, metres per second), and rounds started with the game API's test switches are practice rounds that never submit. An admin key lets `scoreboard/moderate.sh` list and delete rows and read a seven-day audit log of every session and submission. Setup, limits and the honest caveat (a static site cannot stop someone who reads the script) are in `scoreboard/README.md`.
- **Eleanor.** Type `gonein60seconds` (or run it in the terminal). `js/race.js` loads on demand: a pseudo-3D racer in Canabalt greys with a sixty-second clock. Procedural track with curves, hills, ramps that launch the car over traffic, and tunnels that muffle the music; parallax skyline with lit windows and rooftop neon signs (localhost. on roadside buildings, flickering; LOCALHOST DEVELOPMENT on the skyline); near-miss bonuses, spin-outs, screen shake, dust and speed lines. Hold (finger, mouse, Space or Up) to drive, slide or arrows to steer; P pause, M mute, R again, Esc back; two-tap quit and a mute button on touch. Music is a procedural WebAudio loop (D minor arpeggio, driving bass, kick, snare, hats) with an engine note that follows the speed. Eleanor herself is a low-poly 3D model rendered in software (extruded fastback profile, greenhouse, eight-sided wheels, flat shading, painter-sorted faces with back-face culling) that rolls, yaws and pitches. Runs end with the same name entry and worldwide top 10 as Battle City (ranked by metres, then top speed); the worker keeps the two boards apart with a `game` parameter, and `scores eleanor` prints it in the terminal.
