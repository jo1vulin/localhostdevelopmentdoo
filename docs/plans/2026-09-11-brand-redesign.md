...
- **Scoreboard.** At the end of a round with a score, the player enters a name (remembered for next time) and the result goes to the scoreboard: top 10 with rank, name, score and tanks, the player's own entry highlighted; `scores` in the terminal prints it. Persistence is behind one meta tag: while `<meta name="score-api">` is empty the board lives in the visitor's browser; with an endpoint it is shared worldwide and survives deployments. The endpoint is a two-call contract (GET top 10, POST an entry) with two ready implementations in `scoreboard/`: a Cloudflare Worker with KV (validation, CORS allowlist, per-IP rate limit) and a Google Apps Script writing to a sheet. Setup steps are in `scoreboard/README.md`.
